"""从设置中的 MCP 描述拉取工具元数据；MVP 实现文档化 mock transport。"""

from typing import Any

from app.schemas.tools import ToolItem

# 约定：非密钥 JSON 中的 MCP 项可带 transport，mock 用于无外部 Server 的验收与单测。
...
def tools_from_mcp_settings(mcp_servers: list[Any]) -> list[ToolItem]:
    ...
    """
    # 1. 遍历 MCP Server 元数据（名称、URL、是否启用等，无密钥）
    out: list[ToolItem] = []
    for raw in mcp_servers:
        ...
        transport = str(raw.get("transport") or "").lower()  # 通信传输方式
        ...
        if not is_mock or not isinstance(tool_specs, list):
            # 2. 未实现 stdio/SSE 真实 MCP 拉取时跳过（避免误报空列表为错误）
            continue
