...
import { createSession } from '@/modules/sessions/api/sessions'
import { createTask } from '@/modules/tasks/api/tasks'
import { STATUS_COLOR_MAP, STATUS_LABEL_MAP } from '@/core/lib/constants'
import { formatRelativeTime } from '@/core/lib/format'
    ...
    <div className="flex min-h-0 flex-1 flex-col overflow-y-auto">
      ...
      <div className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-8 px-6 py-8">
        ...
        <section className="fa-reveal fa-reveal-delay-1">
          ...
          {data && data.items.length > 0 && (
            <ul className="fa-card divide-y divide-neutral-200 overflow-hidden p-0">
              {data.items.map((task) => {
                ...
                return (
                  <li key={task.id}>
                    <Link
                      to={`/tasks/${task.id}`}
                      className="flex items-center gap-4 px-4 py-3 transition-[background-color] duration-200 hover:bg-white"
                    >
                      <span
