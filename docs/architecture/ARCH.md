...
HTTP 层（api/）→ 服务层（services/）→ 仓储层（repositories/）→ 模型层（models/）
       ...
       modules/workflow（LangGraph 编排）⟷ modules/tools（统一工具注册表）
                     ↕
    modules/planning · modules/execution · modules/memory（与 PRD 四能力对齐，可独立扩展）
```
...
│   │   ├── memory/                        # 记忆域扩展点（会话消息持久化现由 repositories）
│   │   ├── tools/                         # 工具：registry、builtin、mcp_sources、skill_sources
│   │   ├── execution/                     # 执行：executor 节点、流式回复、think/answer 拆分
...
| 文件 | 核心职责 |
|------|----------|
| `task_service.py` | 创建任务（写 tasks + messages）→ 异步调度 `workflow` 编译图；列表与详情；取消/终态 |
...
#### 3.9 `app/modules/workflow` + `planning` + `execution` — 编排与四模块中的规划/执行

| 位置 | 职责 |
|------|------|
| `workflow/state.py` | `AgentState(TypedDict)`：任务 id、用户消息、计划步骤、重规划计数、终态等 |
| `workflow/graph.py` | 构建并编译 LangGraph：`planner` → `executor` → 条件边 → `replan_record` 或 `END` |
| `planning/llm.py` | 规划用 LLM 调用与计划 JSON 解析 |
| `planning/nodes.py` | `planner_node`、`replan_record_node`、测试用 `initial_force_replan_budget` |
| `execution/nodes.py` | `executor_node`、路由与流式 delta 批量落库 |
| `execution/llm_reply.py` | 执行阶段流式助手回复 |
| `execution/stream_split.py` | think/answer 流式分段 |

**LangGraph 最小图结构**（与实现一致）：

```
[START] → planner → executor → (replan?) → replan_record → planner … → [END]
```
...
用户请求 POST /api/v1/tasks
  → api/v1/tasks.py（校验 Schema）
    → services/task_service.py（创建 Task + Message，独立会话提交）
      ...
      → asyncio.create_task：独立 AsyncSession 内执行 LangGraph
        → modules/workflow/graph.py（get_compiled_agent_graph）
          → modules/planning/nodes.py（planner / replan_record）
          → modules/planning/llm.py（计划 JSON）
          → modules/execution/nodes.py（executor、流式增量写库）
          → modules/tools/registry.py（工具元数据与调用）
        → 各节点经 repositories/event_repository.py 追加 task_events（seq 单调）
